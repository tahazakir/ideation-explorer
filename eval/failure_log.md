...
- **what_triggered_the_problem**: Run `ideation_explorer.main --assignment dummy --rooms 2 --depth 3 --options 3` (test case C1). Same shape also reproduced on C4 (lit_review) and C5 (multi_agent).
...
- **evidence**: [outputs/runs/dummy.json](../outputs/runs/dummy.json), [outputs/runs/lit_review.json](../outputs/runs/lit_review.json)
...
- **date**: 2026-04-21
- **version_tested**: v0.2
- **what_triggered_the_problem**: Run on the deliberately vague spec (test case C3: `Make something cool with AI`, two trivial constraints). Designed to stress the system: when the brief contains no real constraints, every option looks roughly viable to the consultant.
...
- **fix_attempted**: Same v0.3 `--min-margin` gate covers this case (margin 0.064 still under default 0.05? — no, 0.064 > 0.05; so for vague-style runs we recommend running with `--min-margin 0.08` until we calibrate). v0.3 also propagates `est_days / deadline_days` into the trace; a future v0.4 could refuse when the recommended plan consumes >80% of the deadline window.
- **current_status**: Partially resolved. The margin gate fires for the dummy run but not vague at the default threshold. Calibrating `--min-margin` per assignment style is an open issue tracked in `version_notes.md`.
- **evidence**: [outputs/runs/vague.json](../outputs/runs/vague.json)
...
- **what_happened**: No production incident, but a clear cost-governance gap. C1 burned 40 LLM calls / 19k tokens for a 7-day web-app brief; that's fine, but the system offered no way to say "spend at most 10 consultations and tell me what you got."
...
- **current_status**: Resolved. Demo command to reproduce the budget-hit + governance refusal:
  ...
  python -m ideation_explorer.main --assignment vague --rooms 3 --depth 3 --options 3 \
      ...
      --out outputs/runs/vague_budget_capped.json --plan-out outputs/recommended_plans/vague_budget_capped.json
  ```
...
- **date**: 2026-04-25
- **version_tested**: v0.3
- **what_triggered_the_problem**: Comparing consultant v1 scope_fit predictions against Professor Anand's ground-truth ratings on 4 calibration plans (test case C7).
